...
void FastText::addInputVector(Vector& vec, int32_t ind, std::minstd_rand& rng) const {
  vec.addRow(*input_, ind, rng, args_->binarization != binarization_name::sbc);
}
...
void FastText::getWordVector(Vector& vec, const std::string& word, std::minstd_rand& rng) const {
  const std::vector<int32_t>& ngrams = dict_->getSubwords(word);
  vec.zero();
  for (int i = 0; i < ngrams.size(); i++) {
    addInputVector(vec, ngrams[i], rng);
  }
  if (ngrams.size() > 0) {
...
void FastText::getSubwordVector(Vector& vec, const std::string& subword, std::minstd_rand& rng) const {
  vec.zero();
  int32_t h = dict_->hash(subword) % args_->bucket;
  h = h + dict_->nwords();
  addInputVector(vec, h, rng);
}
...
void FastText::getSentenceVector(std::istream& in, fasttext::Vector& svec, std::minstd_rand& rng) {
  svec.zero();
  if (args_->model == model_name::sup) {
    std::vector<int32_t> line, labels;
    dict_->getLine(in, line, labels);
    for (int32_t i = 0; i < line.size(); i++) {
      addInputVector(svec, line[i], rng);
    }
    if (!line.empty()) {
...
std::vector<std::pair<real, std::string>> FastText::getNN(
  ...
  for (int32_t i = 0; i < dict_->nwords(); i++) {
    ...
    if (banSet.find(word) == banSet.end()) {
      real dp = wordVectors.dotRow(query, i, rng);
