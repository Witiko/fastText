...
void Vector::addRow(const Matrix& A, int64_t i, real a, std::minstd_rand& rng, bool binarization) {
  assert(i >= 0);
  assert(i < A.size(0));
  assert(size() == A.size(1));
  A.addRowToVector(*this, i, a, rng, binarization);
}

void Vector::addRow(const Matrix& A, int64_t i, std::minstd_rand& rng, bool binarization) {
  assert(i >= 0);
  assert(i < A.size(0));
  assert(size() == A.size(1));
  A.addRowToVector(*this, i, rng, binarization);
}
